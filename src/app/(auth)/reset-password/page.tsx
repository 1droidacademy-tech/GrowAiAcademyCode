...
function ResetPasswordForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      console.log("Sending request to /api/auth/reset-password with token:", token);
  ...
  return (
    <main className="min-h-screen bg-[#F8FAFC] flex font-sans pt-20">
      ...
      <div className="flex-1 flex flex-col justify-center px-8 sm:px-16 lg:px-24">
        <Suspense fallback={<div className="max-w-md w-full mx-auto text-center font-bold text-slate-400">Syncing with nebula...</div>}>
          <ResetPasswordForm />
        </Suspense>
      </div>
    </main>
